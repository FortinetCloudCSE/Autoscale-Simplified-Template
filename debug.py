...
class FgtConf:
    ...
    def do_launch(self):
        ...
        # Upload license
        self.logger.info(f"MDW: do_launch nl = {self.need_license}, fgt_lic_mgmt = {self.fgt_lic_mgmt}, fmg_integration = {self.fmg_integration}")
        ...
        if self.fgt_lic_mgmt != "fmg":
            time.sleep(10)
            self.logger.info(f"MDW: do_launch lic_mgmt is not fmg")
            self.intf_setting = json.loads(os.getenv('network_interfaces'))
            self.logger.info(f"MDW: do_launch intf_setting = {self.intf_setting}")
            ...
            self.logger.info(f"MDW: before gen_config_content, fgt_primary_ip = {self.fgt_primary_ip}, fgt_primary_port = {self.fgt_primary_port}")
            config_content = self.gen_config_content(self.fgt_vm_id)
            self.logger.info(f"MDW: do_launch config_content = {config_content}")
            b_succ = self.upload_config(config_content, fgt_private_ip)
            self.logger.info(f"MDW: Post upload_config b_succ = {b_succ}")
...
# License
    def upload_license(self, fgt_private_ip, fgt_vm_id):
        ...
        self.logger.info(f"MDW: get_license return value: lt = {license_type} lc = {license_content} sn = {sn_or_file_name} ")
        if license_type == "":
        ...
        if license_type == "token":
            ...
            self.logger.info(f"MDW: upload license payload: {payload} ")
            b_succ, response = self.invoke_lambda(payload, "fgt")
            if b_succ:
                self.logger.info(f"MDW: success b_succ = {b_succ} response = {response}")
                used_sn_map = self.get_used_sn_map()
    ...
    def get_license(self, fgt_vm_id):
        ...
        oauth_token = self.get_fortiflex_oauth_token()
        self.logger.info(f"MDW: Post FortiFlex token token = {oauth_token} ")
        if not oauth_token:
        ...
        if available_sn_list:
            for cur_sn in available_sn_list:
                ...
                if vm_token:
                    self.logger.info(f"MDW: Sucess generating vm token = {vm_token} with serial number = {cur_sn}")
