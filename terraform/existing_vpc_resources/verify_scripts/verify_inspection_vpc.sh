...
if is_tfvar_true "create_nat_gateway_subnets" "$TFVARS_FILE"; then
    print_info "Internet mode: NAT Gateway (create_nat_gateway_subnets = true)"
else
    print_info "Internet mode: EIP per-instance (create_nat_gateway_subnets = false)"
fi
...
# NAT Gateway subnets (if create_nat_gateway_subnets = true)
if is_tfvar_true "create_nat_gateway_subnets" "$TFVARS_FILE"; then
    for AZ_NUM in 1 2; do
        if [ $AZ_NUM -eq 1 ]; then
            FULL_AZ="$FULL_AZ1"
    ...
    print_skip "NAT Gateway subnets check skipped (create_nat_gateway_subnets = false)"
fi
...
# GWLBE route table check - per AZ, always present
print_info "Checking GWLBE route tables..."
for AZ_NUM in 1 2; do
    GWLB_RT_ID=$(aws ec2 describe-route-tables \
        --region "$AWS_REGION" \
        --filters "Name=vpc-id,Values=${VPC_ID}" "Name=tag:Name,Values=${CP}-${ENV}-inspection-gwlbe-rt-az${AZ_NUM}" \
        --query 'RouteTables[0].RouteTableId' \
        --output text 2>/dev/null)
    if [ "$GWLB_RT_ID" == "None" ] || [ -z "$GWLB_RT_ID" ]; then
        print_fail "GWLBE route table AZ${AZ_NUM} not found (${CP}-${ENV}-inspection-gwlbe-rt-az${AZ_NUM})"
    else
        print_pass "GWLBE route table AZ${AZ_NUM} exists: $GWLB_RT_ID"
    fi
done
# Legacy single-table check for post-autoscale GWLB RT (created by autoscale_template)
GWLB_RT_ID=$(aws ec2 describe-route-tables \
    --region "$AWS_REGION" \
...
if is_tfvar_true "create_nat_gateway_subnets" "$TFVARS_FILE"; then
    print_info "Verifying NAT Gateways (create_nat_gateway_subnets = true)..."

    for AZ_NUM in 1 2; do
        NAT_GW=$(aws ec2 describe-nat-gateways \
            --region "$AWS_REGION" \
            --filter "Name=tag:Name,Values=${CP}-${ENV}-inspection-nat-gw-az${AZ_NUM}" "Name=state,Values=available" \
    ...
    print_skip "NAT Gateway check skipped (create_nat_gateway_subnets = false)"
fi
