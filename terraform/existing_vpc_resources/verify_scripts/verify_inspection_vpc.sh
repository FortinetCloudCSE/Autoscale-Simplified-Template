...
for AZ_NUM in 1 2; do
    ...
    if [ "$PUBLIC_RT_ID" != "None" ] && [ -n "$PUBLIC_RT_ID" ]; then
        ...
        if verify_route_exists "$PUBLIC_RT_ID" "0.0.0.0/0" "$AWS_REGION"; then
            ...
            if [[ "$TARGET" == igw-* ]]; then
                print_pass "Public AZ${AZ_NUM}: Default route (0.0.0.0/0) points to Internet Gateway: $TARGET"
            else
                print_fail "Public AZ${AZ_NUM}: Default route exists but does not point to IGW: $TARGET"
            fi
        else
            print_fail "Public AZ${AZ_NUM}: Default route (0.0.0.0/0) not found in public route table"
        fi
...
for AZ_NUM in 1 2; do
    ...
    if [ "$PRIVATE_RT_ID" != "None" ] && [ -n "$PRIVATE_RT_ID" ]; then
        ...
        # Check for NAT Gateway route if applicable
        if [ "$ACCESS_MODE" == "nat_gw" ]; then
            if verify_route_exists "$PRIVATE_RT_ID" "0.0.0.0/0" "$AWS_REGION"; then
                ...
                if [[ "$TARGET" == nat-* ]]; then
                    ...
                    print_fail "Private AZ${AZ_NUM}: Default route exists but not to NAT Gateway: $TARGET"
                fi
            else
                print_info "Private AZ${AZ_NUM}: Default route not found (may not be created yet)"
            fi
        else
            # In EIP mode, there should be no default route
            if verify_route_exists "$PRIVATE_RT_ID" "0.0.0.0/0" "$AWS_REGION"; then
                print_fail "Private AZ${AZ_NUM}: Default route exists but shouldn't (access_internet_mode = eip)"
            else
                print_pass "Private AZ${AZ_NUM}: No default route (correct for eip mode)"
            fi
        fi
    else
...
# GWLB route table should NOT exist yet
print_info "Verifying GWLB route table does NOT exist (will be created by autoscale module)..."
...
if [ "$GWLB_RT_ID" == "None" ] || [ -z "$GWLB_RT_ID" ]; then
    print_pass "GWLB route table does not exist yet (correct - will be created by autoscale module)"
else
    print_fail "GWLB route table exists but should not: $GWLB_RT_ID (should be created by autoscale module)"
fi
