...
if [ "$TGW_RT_ID" != "None" ] && [ -n "$TGW_RT_ID" ]; then
    ...
    # Determine expected target based on boolean flags
    if is_tfvar_true "enable_build_management_vpc" "$TFVARS_FILE" && \
       is_tfvar_true "enable_management_tgw_attachment" "$TFVARS_FILE" && \
       is_tfvar_true "enable_build_existing_subnets" "$TFVARS_FILE"; then
        # Should point to Management VPC attachment - query by VPC ID
        ...
        EXPECTED_TARGET=$(aws ec2 describe-transit-gateway-attachments \
            --region "$AWS_REGION" \
            ...
            --output text 2>/dev/null)
        TARGET_NAME="Management VPC"
    else
        # Should point to Inspection VPC attachment - query by VPC ID
        ...
        EXPECTED_TARGET=$(aws ec2 describe-transit-gateway-attachments \
            ...
            --query 'TransitGatewayAttachments[0].TransitGatewayAttachmentId' \
            --output text 2>/dev/null)
        TARGET_NAME="Inspection VPC"
    fi

    if [ -z "$DEFAULT_ROUTE_TARGET" ]; then
        print_fail "Default route (0.0.0.0/0) not found in TGW route table"
    elif [ "$DEFAULT_ROUTE_TARGET" == "$EXPECTED_TARGET" ]; then
        print_pass "Default route points to $TARGET_NAME: $DEFAULT_ROUTE_TARGET"
    else
        print_fail "Default route does not point to expected $TARGET_NAME attachment. Expected: $EXPECTED_TARGET, Got: $DEFAULT_ROUTE_TARGET"
    fi
else
